...
from src.util import Logger, forward_hook, count_autograd_graph
from src.agenv.zxopt_agent_base import AgentGNNBase, CategoricalMasked, PPOAgent, PPGAgent
from src.agenv.zxopt_agent1 import AgentGNN1
from src.agenv.zxopt_agent3 import AgentGNN3

class EthernalAgentBase(AgentGNNBase,ABC):
    def __init__(
        self,
        envs,
        device,
    ...
    def get_next_action(self, graph, info, action=None, device="cpu", testing=False, mask_stop=False):
        ...
        for b in range(policy_obs.num_graphs):
            ...
            if info[b]['nstep'] >= self.nstep:
                ...
                # 
                DUMMY_LOGIT = 10.0
                ids = torch.cat((ids,torch.tensor([self.shape * (self.shape + 1) + 1]).to(device)))
                probs = torch.tensor([-1e8]*probs.shape[0] + [DUMMY_LOGIT]).to(device)
                batch_logits[b, : probs.shape[0]] = probs.flatten()
                act_mask[b, : probs.shape[0]] = torch.tensor([False] * (probs.shape[0]-1) + [True])
                action_nodes_mask = torch.where(ids != -1)[0].to(device)
    ...
    def get_value(self, graph, info):
        ...
        values = self.critic(policy_obs)
        values = values.squeeze(-1)
        return values# 


# no stop, n step
class PPOEthernalAgent(EthernalAgentBase,PPOAgent):
    def __init__(
        self,
        envs,
        device,
        ...
        **kwargs,
        ...
        self.critic_head_aggregation = geom_nn.aggr.AttentionalAggregation(
...
class PPGEthernalAgent(EthernalAgentBase,PPGAgent):
    def __init__(
        self,
        envs,
        device,
